...
def scale_face(bm, face, scale_x, scale_y, scale_z):
    ...
    face_space.invert()
    bmesh.ops.scale(
        bm, vec=Vector((scale_x, scale_y, scale_z)), space=face_space, verts=face.verts
    )
...
def get_aspect_ratio(face):
    if not face.is_valid:
        return 1.0
    face_aspect_ratio = max(
        0.01, face.edges[0].calc_length() / face.edges[1].calc_length()
    )
...
# Returns true if this face is pointing behid the ship
...
def add_exhaust_to_face(bm, face):
    ...
    exhaust_length = uniform(1.2, 1.5)
    scale_outer = 1 / uniform(2.0, 2.1)
    scale_inner = 1 / uniform(3.05, 3.1)
    ...
    for h in range(horizontal_step):
        ...
        for v in range(vertical_step):
            ...
            bmesh.ops.create_cone(
                ...
                matrix=turret_house_mat
                @ Matrix.Translation(
                    Vector((weapon_size * 0.2, 0, -weapon_size))
                ).to_4x4(),
            )
            bmesh.ops.create_cone(
                bm,
...
def add_sphere_to_face(bm, face):
    ...
    sphere_matrix = get_face_matrix(
        face, face.calc_center_bounds() - face.normal * uniform(0, sphere_size * 0.5)
    )
    result = bmesh.ops.create_icosphere(
        bm, subdivisions=3, radius=sphere_size, matrix=sphere_matrix
    )
...
def add_disc_to_face(bm, face):
    ...
    bmesh.ops.create_cone(
        ...
        depth=depth,
        matrix=get_face_matrix(
            face, face.calc_center_bounds() + face.normal * depth * 0.5
        ),
    )
    result = bmesh.ops.create_cone(
        bm,
        ...
        depth=0.0,
        matrix=get_face_matrix(
            face, face.calc_center_bounds() + face.normal * depth * 1.05
        ),
    )
    for vert in result["verts"]:
        for face in vert.link_faces:
...
def create_materials():
    ...
    hull_normal_map = bpy.data.images.load(
        "/Users/ddigiacomo/SpaceshipGenerator/textures/hull_normal.png",
        check_existing=True,
    )
    ...
    hull_lights_diffuse_map = bpy.data.images.load(
        "/Users/ddigiacomo/SpaceshipGenerator/textures/hull_lights_diffuse.png",
        check_existing=True,
    )
    ...
    hull_lights_emessive_map = bpy.data.images.load(
        "/Users/ddigiacomo/SpaceshipGenerator/textures/hull_lights_emit.png",
        check_existing=True,
    )
...
def generate_spaceship(
    ...
    # x, y, z
    scale_vector = Vector(
        (uniform(15.75, 20.0), uniform(5.0, 8.0), uniform(15.75, 20.0))
    )
    ...
    for face in bm.faces[:]:
        if abs(face.normal.x) > 0.5:
            hull_segment_length = uniform(2.3, 5)
            ...
            for i in hull_segment_range:
                ...
                """
                    if random() > 0.5:
                        ...
                        bmesh.ops.translate(
                            bm, vec=sideways_translation, verts=face.verts
                        )
    ...
    if create_asymmetry_segments:
        for face in bm.faces[:]:
            ...
            if get_aspect_ratio(face) > 2:
                continue
            if random() > 0.85:
                hull_piece_length = uniform(0.1, 0.4)
                for i in range(
                    randrange(num_asymmetry_segments_min, num_asymmetry_segments_max)
                ):
    ...
    if create_face_detail:
        ...
        for face in bm.faces[:]:
            ...
            if is_rear_face(face):  # rear face
                if not engine_faces or val > 0.65:
    ...
    if allow_horizontal_symmetry and random() > 0.5:
        bmesh.ops.symmetrize(
            bm, input=bm.verts[:] + bm.edges[:] + bm.faces[:], direction="Y"
        )
    ...
    if allow_vertical_symmetry and random() > 0.5:
        bmesh.ops.symmetrize(
            bm, input=bm.verts[:] + bm.edges[:] + bm.faces[:], direction="Z"
        )
...
if __name__ == "__main__":
    ...
    if generate_single_spaceship:
        ...
        bpy.ops.mesh.add_pipes(seed=randint(100,1000), number=8, support_period=4, radius=0.70, offset=1.5)
